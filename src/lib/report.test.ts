...
describe("trendSeries", () => {
  ...
  it("compounds smoothing across gaps so it does not lag after a break", () => {
    const daily = trendSeries(
      dates("2026-01-01", 15).map((date, i) => ({
        date,
        value: i === 0 ? 90 : 88,
      })),
    );
...
describe("ratePerWeek", () => {
  it("reads a steady loss as a weekly rate", () => {
    const points = dates("2026-01-01", 15).map((date, i) => ({
      date,
      value: 90 - i * 0.1,
    })); // 0.7 kg/week
  ...
  it("only fits the recent window", () => {
    ...
    const recent = dates("2026-02-01", 14).map((date, i) => ({
      date,
      value: 90 - i * 0.1,
    }));
...
describe("summarize", () => {
  ...
  it("measures intake against the budget, exercise aside", () => {
    // 2300 eaten and 300 burned is a day 300 over, not a day on budget.
    const s = summarize([day("2026-01-01", 2300, 300)], budget);
    expect(s.avgFood).toBe(2300);
    expect(s.avgExercise).toBe(300);
    expect(s.avgVsBudget).toBe(300);
    expect(s.daysOver).toBe(1);
  });

  it("counts days over budget and totals the surplus", () => {
    const s = summarize(
      [day("2026-01-01", 2500), day("2026-01-02", 1700)],
      budget,
    );
    expect(s.daysOver).toBe(1);
...
describe("mealAverages", () => {
  it("averages each meal across logged days", () => {
    ...
    const avgs = Object.fromEntries(
      mealAverages(days).map((m) => [m.meal, m.average]),
    );
