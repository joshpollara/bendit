...
/**
 * Compares two groups of days and returns the difference in mean intake,
...
function compare(
  ...
  if (spread > 0 && Math.abs(difference) / spread < MIN_EFFECT)
    return undefined;
...
export function findPatterns(
  ...
  const weekendResult = compare(weekend.map(intake), weekday.map(intake));
  if (weekendResult) {
    patterns.push({
      id: "weekend",
  ...
  if (byWeekday.length >= 3) {
    const scored = byWeekday
      .map((group) => ({ ...group, average: mean(group.days.map(intake)) }))
      .sort((a, b) => b.average - a.average);
    const highest = scored[0];
    const rest = logged.filter(
      (d) => weekdayIndex(d.date) !== WEEKDAY_NAMES.indexOf(highest.name),
    );
    const result = compare(highest.days.map(intake), rest.map(intake));
    if (result) {
      patterns.push({
        id: "weekday-high",
  ...
  if (withProtein.length >= MIN_GROUP * 2) {
    const threshold =
      proteinTargetG ??
      [...withProtein].map((d) => d.protein).sort((a, b) => a - b)[
        Math.floor(withProtein.length / 2)
      ];
    const high = withProtein.filter((d) => d.protein >= threshold);
    const low = withProtein.filter((d) => d.protein < threshold);
    const proteinResult = compare(high.map(intake), low.map(intake));
    if (proteinResult) {
      patterns.push({
        id: "protein",
  ...
  const breakfastResult = compare(
    withBreakfast.map(intake),
    withoutBreakfast.map(intake),
  );
  if (breakfastResult) {
    patterns.push({
      id: "breakfast",
